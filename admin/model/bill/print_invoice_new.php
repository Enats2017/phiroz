...
class ModelBillPrintInvoice extends Model {
	...
	public function getall_transaction_group($data = array()) {
		...
		$sql .= " GROUP BY `horse_id`, `medicine_id`, `dot` ASC";
		//echo $sql;exit;
	...
	public function get_transaction_data($transaction_id) {
		...
		$sql .= " GROUP BY treatment_name ASC ";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function get_transaction_data_enats($transaction_id) {
		$sql = "SELECT transaction_id,medicine_id ,treatment_name ,dot,month,year,medicine_doctor_id,medicine_name ,SUM(medicine_quantity) AS med_qty, SUM(medicine_total) AS med_total FROM `" . DB_PREFIX . "transaction` WHERE `transaction_id` IN($transaction_id) AND `cancel_status` = '0' AND treatment_name <> '' "; 
		$sql .= " GROUP BY treatment_name,dot ASC ";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function getbillowners($data = array()) {
		$sql = "SELECT `id`, `bill_id`, `horse_id`,`ref_id`, `owner_id`, `owner_amt`, `trainer_id`, `owner_share`, `owner_amt`, `owner_amt_rec`, `doctor_id`, `month`, `year`, `dop`, `accept`, `cheque_no`, `total_amount`, `invoice_date`, `transaction_type`, `owner_amt_rec`, `payment_status`, `owner_code`, `batch_id` FROM `" . DB_PREFIX . "bill_owner` WHERE `cancel_status` = '0' "; 
	...
	public function getbillowners_owner($data = array()) {
		$sql = "SELECT `id`, `bill_id`, `horse_id`,`ref_id`,`payment_type`,`owner_id`, `owner_amt`, `trainer_id`, `owner_share`, `owner_amt`, `owner_amt_rec`, `doctor_id`, `month`, `year`, `dop`, `accept`, `cheque_no`, `total_amount`, `invoice_date`, `transaction_type`, `owner_amt_rec`, `payment_status`, `owner_code`, `batch_id` FROM `" . DB_PREFIX . "bill_owner` WHERE `cancel_status` = '0' "; 
		if (isset($data['filter_name_id']) && !empty($data['filter_name_id'])) {
			$sql .= " AND `horse_id` = '" . (int)$data['filter_name_id'] . "'";
		} 
