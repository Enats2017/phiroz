...
class ModeltransactionCredit extends Model {
	...
	public function deleteowner($owner_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "credit WHERE owner_id = '" . (int)$owner_id . "'");
	}	

	public function getowner($owner_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "credit WHERE owner_id = '" . (int)$owner_id . "'");
		return $query->row;
	}
	...
	public function getowners($data = array()) {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "credit WHERE 1=1 ";
		...
		if (isset($data['filter_responsible_person']) && !empty($data['filter_responsible_person'])) {
			...
			$sql .= " AND LOWER(owner_name) LIKE '%" . $this->db->escape($data['filter_responsible_person']) . "%'";
		}
	...
	public function getTotalowners($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "credit WHERE 1=1 ";
