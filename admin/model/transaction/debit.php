...
class ModeltransactionDebit extends Model {
	...
	public function deleteowner($owner_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "debit WHERE owner_id = '" . (int)$owner_id . "'");
	}	

	public function getowner($owner_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "debit WHERE owner_id = '" . (int)$owner_id . "'");
		return $query->row;
	}
	...
	public function getowners($data = array()) {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "debit WHERE 1=1 ";
	...
	public function getTotalowners($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "debit WHERE 1=1 ";
