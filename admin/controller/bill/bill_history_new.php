...
class ControllerBillBillHistory extends Controller { 
	...
	public function printinvoice(){
		...
		foreach ($bill_groups as $bkey => $bvalue) {
			...
			foreach ($transaction_ids as $tkey => $tvalue) {
				$t_id .= $tvalue['transaction_id'].',';
			}
			$trance_id = rtrim($t_id,',');
				$transaction_data = $this->model_bill_print_invoice->get_transaction_data_enats($trance_id);
				$final_transaction = array();
				foreach ($transaction_data as $tkey => $pvalue) {
					$final_transaction[] = array(
					'bill_id' 	  	=> $bvalue['bill_id'],
					'transaction_id' 	  	=> $pvalue['transaction_id'],
					'medicine_id' 	  	=> $pvalue['medicine_id'],
					'medicine_name' 	  	=> $pvalue['medicine_name'],
					'treatment_name' 	  	=> $pvalue['treatment_name'],
					'medicine_quantity'	  	=> $pvalue['med_qty'],
					'medicine_total'	  	=> $pvalue['med_total'],
					'dot'	  	=> date('M d, Y', strtotime($pvalue['dot'])),
					'month'	  	=> $pvalue['month'],
					'year'	  	=> $pvalue['year'],
				);
				// echo "<pre>";
				// print_r($final_transaction);
				//  exit;
				...
				// $sorth1 = array();
				// foreach($final_transaction as $s=>$p) {
				//    $sorth1['dot'][$s] = $p['dot'];
				// }
				// array_multisort($sorth1['dot'], SORT_ASC,$final_transaction);
				// $data['final_transaction'] = $final_transaction;

				$medicine_datas =$this->db->query("SELECT * FROM `" . DB_PREFIX . "transaction` WHERE `transaction_id` IN($trance_id) AND `cancel_status` = '0' AND treatment_name = '' ")->rows;
				$medi_transaction = array();
				foreach ($medicine_datas as $mkey => $mvalue) {
					$medi_transaction[] = array(
					'bill_id' 	  	=> $bvalue['bill_id'],
					'transaction_id' 	  	=> $mvalue['transaction_id'],
					'medicine_id' 	  	=> $mvalue['medicine_id'],
					'medicine_name' 	  	=> $mvalue['medicine_name'],
					'treatment_name' 	  	=> $mvalue['treatment_name'],
					'medicine_quantity'	  	=> $mvalue['medicine_quantity'],
					'medicine_total'	  	=> $mvalue['medicine_total'],
					'dot'	  	=> date('M d, Y', strtotime($mvalue['dot'])),
					'month'	  	=> $mvalue['month'],
					'year'	  	=> $mvalue['year'],
				);
				...
				$bill_final_aryss = array_merge($final_transaction, $medi_transaction);


				$sorth = array();
				foreach($bill_final_aryss as $k=>$v) {
				   $sorth['dot'][$k] = $v['dot'];
				   $sorth['transaction_id'][$k] = $v['transaction_id'];
				}

				array_multisort($sorth['dot'], SORT_ASC, $sorth['transaction_id'], SORT_ASC,$bill_final_aryss);
				$data['bill_final_aryss'] = $bill_final_aryss;

				// echo "<pre>";
				// print_r($data['bill_final_aryss']);
				// exit;
							...
							if($filter_owner_id != ''){ 
								if($ovalue['owner'] == $filter_owner_id){
									if($ovalue['share']) {
										...
										// $final_owner[$okey]['medicine_datas'] = $medi_transaction;
										$final_owner[$okey]['medicine_datas'] = $data['bill_final_aryss'];
		...
		if($final_data){
			...
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$template->data['base'] = "http://5.189.160.61/phiroz/admin/";
			} else {
				$template->data['base'] = "http://5.189.160.61/phiroz/admin/";
			}
			// echo $html;
	...
	public function printinvoiceone(){
		...
		$trance_id = rtrim($t_id,',');
		$transaction_data = $this->model_bill_print_invoice->get_transaction_data_enats($trance_id);
		foreach ($transaction_data as $tkey => $svalue) {

			$final_transaction[$tkey]['bill_id'] = $filter_bill_id;
			$final_transaction[$tkey]['transaction_id'] = $svalue['transaction_id'];
			$final_transaction[$tkey]['medicine_id'] = $svalue['medicine_id'];
			$final_transaction[$tkey]['medicine_name'] = $svalue['medicine_name'];
			$final_transaction[$tkey]['treatment_name'] = $svalue['treatment_name'];
			$final_transaction[$tkey]['medicine_quantity'] = $svalue['med_qty'];
			$final_transaction[$tkey]['medicine_total'] = $svalue['med_total'];
			$final_transaction[$tkey]['dot'] = date('M d, Y', strtotime($svalue['dot']));
			$final_transaction[$tkey]['month'] = $svalue['month'];
			$final_transaction[$tkey]['year'] = $svalue['year'];

			$medicine_doctor_id = $svalue['medicine_doctor_id'];
		}

		// $sorth = array();
		// foreach($final_transaction as $k=>$v) {
		//    $sorth['bill_id'][$k] = $v['bill_id'];
		//    $sorth['dot'][$k] = $v['dot'];
		// }
		// array_multisort($sorth['dot'], SORT_ASC,$final_transaction);
		// $data['final_transaction'] = $final_transaction;

		$medicine_datas =$this->db->query("SELECT * FROM `" . DB_PREFIX . "transaction` WHERE `transaction_id` IN($trance_id) AND `cancel_status` = '0' AND treatment_name = '' ORDER BY dot ASC")->rows;
			$medi_transaction = array();
			foreach ($medicine_datas as $mkey => $mvalue) {
				$medi_transaction[] = array(
				'bill_id' 	  	=> $filter_bill_id,
				'transaction_id' 	  	=> $mvalue['transaction_id'],
				'medicine_id' 	  	=> $mvalue['medicine_id'],
				'medicine_name' 	  	=> $mvalue['medicine_name'],
				'treatment_name' 	  	=> $mvalue['treatment_name'],
		...
		if(isset($horse_data['horse_id']) && $horse_data['horse_id'] != '0' && $horse_data['horse_id'] != ''){
			...
			foreach ($owner_data as $okey => $ovalue) {
				...
				// echo "<pre>";
				// print_r($final_owner[$okey]['medicine_datas']);
			}
				 // exit;
		}
	...
	public function invoice_mail_html($show_html = 1){
		...
		$medicine_doctor_id = 0;
		foreach ($transaction_ids as $tkey => $tvalue) {
			$transaction_data = $this->model_bill_print_invoice->get_transaction_data_enats($tvalue['transaction_id']);
