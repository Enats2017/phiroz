...
class ControllerTransactionCredit extends Controller { 
	...
	public function index() {
		$this->language->load('transaction/credit');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_transaction_credit->addowner($this->request->post);
			...
			$this->redirect($this->url->link('transaction/credit', 'token=' . $this->session->data['token'] . $url . '&filter_name=' . $this->request->post['name'], 'SSL'));
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_transaction_credit->editowner($this->request->get['owner_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $owner_id) {
				$this->model_transaction_credit->deleteowner($owner_id);
			}
			...
			$this->redirect($this->url->link('transaction/credit', 'token=' . $this->session->data['token'] . $url, 'SSL'));
	...
	protected function getList() {
		...
		$this->data['insert'] = $this->url->link('transaction/credit/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('transaction/credit/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		...
		$owner_total = $this->model_transaction_credit->getTotalowners($data);

		$results = $this->model_transaction_credit->getowners($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('transaction/credit/update', 'token=' . $this->session->data['token'] . '&owner_id=' . $result['owner_id'] . $url, 'SSL')
			);

			$action[] = array(
				'text' => $this->language->get('text_delete'),
				'href' => $this->url->link('transaction/credit/delete', 'token=' . $this->session->data['token'] . '&owner_id=' . $result['owner_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('transaction/credit', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		...
		$this->template = 'transaction/credit_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['owner_id'])) {
			$this->data['action'] = $this->url->link('transaction/credit/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('transaction/credit/update', 'token=' . $this->session->data['token'] . '&owner_id=' . $this->request->get['owner_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('transaction/credit', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['owner_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$owner_info = $this->model_transaction_credit->getowner($this->request->get['owner_id']);
		}
		...
		$this->template = 'transaction/credit_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validateForm() {
		...
		// }
		// if(isset($this->request->post['name'])){
		// $is_exist = $this->db->query("SELECT *  FROM " . DB_PREFIX . "credit WHERE LOWER(name) = '" . $this->db->escape(strtolower($this->request->post['name'])) . "'")->rows;
		// 	if($is_exist){
		// 		$this->error['name'] = 'Employee Name Exists';	
		//  	}
		// }
