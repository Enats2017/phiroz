<?php
class ControllerReportHorseWiseSur extends Controller { 
	public function index() {  
		$this->language->load('report/horse_wise');

		$this->document->setTitle('Surgery Report');
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/horse_wise_sur', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);
		...
		$this->data['action'] = $this->url->link('report/horse_wise_sur/export', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->data['heading_title'] = 'Surgery Report';
		...
		$pagination->url = $this->url->link('report/horse_wise_sur', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'report/horse_wise_sur.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function export(){
		...
		if($horse_wise){
			...
			$template->data['title'] = 'Surgery Report';
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$template->data['base'] = 'http://64.79.95.89/phiroz/admin/';
			} else {
				$template->data['base'] = 'http://64.79.95.89/phiroz/admin/';
			}
			$html = $template->fetch('report/horse_wise_sur_html.tpl');
			//echo $html;exit;
			$filename = "Surgery_Report.html";
			...
			$this->redirect($this->url->link('report/horse_wise_sur', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	}
