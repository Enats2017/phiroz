<?php
class ControllerReportPaidBills extends Controller { 
	public function index() {  
		$this->language->load('report/paid_bills');
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/paid_bills', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);
		...
		$order_total = $this->model_report_common_report->gettotal_paid_bills($data);
		
		$results = $this->model_report_common_report->getpaid_bills($data);
		...
		$pagination->url = $this->url->link('report/paid_bills', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'report/paid_bills.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function export(){
		...
		if($paid_bills){
			...
			$template->data['final_datas'] = $paid_bills;
			$template->data['month'] = $month;
			$template->data['year'] = $filter_year;
			$template->data['title'] = 'Paid Bills Report';
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$template->data['base'] = HTTPS_SERVER;
			} else {
				$template->data['base'] = HTTP_SERVER;
			}
			$html = $template->fetch('report/paid_bills_html.tpl');
			//echo $html;exit;
			$filename = "Paid_bills.html";
			...
			$this->redirect($this->url->link('report/paid_bills', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	}
}
?>
