...
class ControllerReportMedicineReport extends Controller { 
	public function index() {  
		$this->language->load('report/medicine_report');
		...
		$results = $this->model_report_common_report->getTransactionbyCategory_group($data);
		$order_total = 0;
		foreach ($results as $result) {
			...
			$this->data['horse_wise'][] = array(
				...
				'medicine_total'    => $this->currency->format($result['medicine_total'], $this->config->get('config_currency')),
		...
		$pagination->url = $this->url->link('report/medicine_report', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'report/medicine_report.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function export(){
		...
		if($horse_wise){
			...
			$template->data['title'] = 'Category Wise Medicine Report';
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$template->data['base'] = HTTPS_SERVER;
			} else {
				$template->data['base'] = HTTP_SERVER;
			}
			$html = $template->fetch('report/medicine_report_html.tpl');
			//echo $html;exit;
			$filename = "Category_Wise_Medicine_Report.html";
			...
			$this->redirect($this->url->link('report/medicine_report', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	}
}
?>
