<?php
class ControllerReportHorseWiseShock extends Controller { 
	public function index() {  
		$this->language->load('report/horse_wise');

		$this->document->setTitle('Shock Wave Report');
		...
		$this->data['action'] = $this->url->link('report/horse_wise_shock/export', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->data['heading_title'] = 'Shock Wave Report';
		...
		$this->template = 'report/horse_wise_shock.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function export(){
		...
		if($horse_wise){
			...
			$template->data['title'] = 'Shock Wave Report';
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$template->data['base'] = 'http://64.79.95.89/phiroz/admin/';
			} else {
				$template->data['base'] = 'http://64.79.95.89/phiroz/admin/';
			}
			$html = $template->fetch('report/horse_wise_shock_html.tpl');
			//echo $html;exit;
			$filename = "Shock_Wave_Report.html";
			...
			$this->redirect($this->url->link('report/horse_wise_shock', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	}
